...
Berechnung mit einfacher Multiplikation (in O(n³))
...
def random_mat(dim_of_mat):
    return [random.uniform(-100, 100) for i in range(dim_of_mat**2)]


def zero_mat(dim_of_mat):
    return [0] * dim_of_mat ** 2
...
def calc():
    ...
    with open("Ergebnisse_python.txt", "w") as output:
